...
# ========== 3. Hyperparameters ==========
best_C = 27.9084         # C value for SVR
best_epsilon = 0.0100    # Epsilon value for SVR
best_kernel = 'rbf'      # Kernel type for SVR

# ========== 4. Perform 5-Fold Cross-Validation on Training Set: Print Per-Fold Metrics and Generate OOF ==========
kf = KFold(n_splits=5, shuffle=True, random_state=1)

# Collect per-fold metrics (print and export)
rows = []  # Two rows per fold: Train / Validation metrics
val_mae_list, val_rmse_list, val_r2_list = [], [], []

# OOF: Reserve space for each sample in the training set (validation predictions from corresponding fold)
oof_pred = np.empty_like(y_train, dtype=float)
oof_pred[:] = np.nan

# Optional: Save per-fold validation sample details (for checking/reproducing experiments)
oof_rows = []
...
for fold_id, (tr_idx, val_idx) in enumerate(kf.split(X_train), 1):
    ...
    model_cv = SVR(C=best_C, epsilon=best_epsilon, kernel=best_kernel)  # Use SVR model
    ...
    # Fill OOF predictions (only from the validation subset of this fold)
    ...
    # Used only for mean ± standard deviation statistics (not written to files)
    val_mae_list.append(val_mae)
    val_rmse_list.append(val_rmse)
    val_r2_list.append(val_r2)

# Validation set mean ± standard deviation (based on each fold's validation set)
...
# ========== 5. Export OOF Training Predictions (Features for Stacking Meta-Learner) ==========
oof_df = pd.DataFrame({
    "global_index": idx_train,    # Mapping back to original data row
    "y_true": y_train,
    "y_pred_oof_svmr": oof_pred    # Suggest appending model name for clarity with other base learners
})
# If you need original features (generally, stacking only needs prediction columns), uncomment the following:
...
oof_df.to_csv("oof_predictions_train_svmr.csv", index=False, encoding="utf-8-sig")
print("OOF predictions for training saved as 'oof_predictions_train_svmr.csv'")


# ========== 6. Final Model Training + Test Set Prediction Export ==========
final_model = SVR(C=best_C, epsilon=best_epsilon, kernel=best_kernel)
final_model.fit(X_train, y_train)
joblib.dump(final_model, 'final_svmr_model_refit_on_full_train.pkl')
print("\nFinal model saved as 'final_svmr_model_refit_on_full_train.pkl'")
...
test_df = pd.DataFrame({
    ...
    "y_true": y_test,
    "y_pred_test_svmr": y_pred_test_final
})
...
test_df.to_csv("predictions_test_svmr.csv", index=False, encoding="utf-8-sig")
print("Test set predictions saved as 'predictions_test_svmr.csv'")
